...
const mockedFolderPath = vscode.Uri.parse(
  "scheme:" + path.join(mockedFilesDirPath, "mocked_data"),
  true
).fsPath;

// mocked server
const mockedServer = nock(`${testHost}${testHostPrefix}`).defaultReplyHeaders({
...
suite("Deepcode Extension Tests", function() {
  ...
  test("Pre-test configuring", () => {
    ...
    assert.equal(
      ...
      path.join(mockedFilesDirPath, "mocked_data")
    );
  });
